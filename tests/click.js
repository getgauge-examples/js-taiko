...
const {
    link, click,below,contains,image,above,toRightOf,toLeftOf,button,rightClick,screenshot
} = require('taiko');
...
step("Click an element that contains <text>", async function (text) {
    await screenshot({path:`before-${text}.jpeg`})
    await click(contains(text));
    await screenshot({path:`after-${text}.jpeg`})
});
